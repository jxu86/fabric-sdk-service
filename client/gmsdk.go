...
func (cli *gmClient) Initialize(confBytes []byte) error {
...
func (cli *gmClient) Close() error {
	cli.sdk.Close()
	return nil
}

// ********************************** Channel functions *****************************
func (cli *gmClient) Query(req CcRequest) (cResp CcResponse, err error) {
	r := channel.Request{}
	r.Fcn = req.Fcn
	r.Args = req.Args
...
func (cli *gmClient) Invoke(req CcRequest) (cResp CcResponse, err error) {
	r := channel.Request{}
	r.Fcn = req.Fcn
	r.Args = req.Args
	r.ChaincodeID = req.ChaincodeID
...
func (cli *gmClient) QueryCCData(ccid string, peers []string) (cResp CcResponse, err error) {
...
// ********************************** Ledger functions *****************************
func (cli *gmClient) LedgerInfo(peers []string) (*common.BlockchainInfo, error) {
...
func (cli *gmClient) BlockInfoByTx(txId string, peers []string) (*common.Block, error) {
	fabTxid := fab.TransactionID(txId)
	return cli.cliLedger.QueryBlockByTxID(fabTxid, ledger.WithTargetEndpoints(peers...))
}

func (cli *gmClient) TxInfoById(txId string, peers []string) (*peer.ProcessedTransaction, error) {
	fabTxid := fab.TransactionID(txId)
	return cli.cliLedger.QueryTransaction(fabTxid, ledger.WithTargetEndpoints(peers...))
}

// ********************************** Resmgmt functions *****************************
func (cli *gmClient) ChaincodeInfo(peers []string) (*peer.ChaincodeQueryResponse, error) {
